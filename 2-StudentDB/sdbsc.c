...
{
    // Calculate the byte offset for the student's record
    ...
    // If we read fewer bytes than expected, return not found
...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
    ...
    // Move to the correct position to write new record
    off_t offset = (id - 1) * STUDENT_RECORD_SIZE;
    if (lseek(fd, offset, SEEK_SET) == (off_t)-1)
    {
    ...
    // Populate the student_t structure
    student.id = id;
    strncpy(student.fname, fname, sizeof(student.fname) - 1);
    strncpy(student.lname, lname, sizeof(student.lname) - 1);
    student.fname[sizeof(student.fname) - 1] = '\0'; // Ensure null-termination
    student.lname[sizeof(student.lname) - 1] = '\0'; // Ensure null-termination
    student.gpa = gpa;
...
int del_student(int fd, int id)
{
    ...
    // Write an empty student record to mark the slot as deleted
...
int print_db(int fd)
{
    ...
    // Read each record and print it if valid
    while (read(fd, &student, sizeof(student_t)) == sizeof(student_t))
    {
        if (memcmp(&student, &EMPTY_STUDENT_RECORD, sizeof(student_t)) != 0)
        {
            ...
            // Calculate and print GPA as a float
            float gpa_float = student.gpa / 100.0;
            printf(STUDENT_PRINT_FMT_STRING, student.id, student.fname, student.lname, gpa_float);
        }
    }
    ...
    // If no records were found, output empty message
    if (!has_records)
    {
        printf(M_DB_EMPTY);
    }
