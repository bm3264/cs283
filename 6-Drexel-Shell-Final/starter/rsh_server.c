...
int process_cli_requests(int svr_socket) {
    ...
    while (1) {
        ...
        close(cli_socket);  // Close client socket after execution

        // If client requested server shutdown (`stop-server`)
        if (rc == OK_EXIT) {
            break;
        }
    }
...
int exec_client_requests(int cli_socket) {
    ...
    while (1) {
        ...
        // Check for built-in commands (exit, stop-server)
        ...
        // Parse the command into command_list_t
...
int send_message_string(int cli_socket, char *buff) {
    ssize_t send_len = strlen(buff);
    ssize_t sent_len = send(cli_socket, buff, send_len, 0);
